...
vdpCoordinates function x,y,vdpCommand((VDP_VRAM.PLANEA)|(y*($40*2)+x*2),VDP_CMD.VRAM)
...
vdpSetVideoAddress: macro address, VDPControlPort
    setRAMAddress	address, VDP_CMD.VRAM, VDPControlPort
	endm
	
vdpSetColorAddress: macro address, VDPControlPort
	setRAMAddress	address, VDP_CMD.CRAM, VDPControlPort
	endm

vdpSetVerticalScrollAddress: macro address, VDPControlPort
    setRAMAddress	address, VDP_CMD.VSRAM, VDPControlPort
	endm


vdpSetVideoSpace: macro offset, VDPControlPort
    move.l  #(((offset*32)&$3FFF)<<16)|(((offset*32)&$C000)>>14)|(VDP_CMD.VRAM),VDPControlPort
	endm
	
vdpSetColorSpace: macro offset, VDPControlPort
	move.l  #(((offset*2)&$3FFF)<<16)|(((offset*2)&$C000)>>14)|(VDP_CMD.CRAM),VDPControlPort
	endm
...
dmaCommandLength function length,((VDP_REG.DMALEN_H|((length&$FF00)>>8))<<16)|($9300|(length&$FF))
dmaCommandSrcLow function source,((VDP_REG.DMASRC_M|((source&$FF00)>>8))<<16)|($9500|(source&$FF))
dmaCommandSrcHigh function source,VDP_REG.DMASRC_H|(((source&$FF0000)>>16)&$7F)
...
dmaFillVRAM macro byte, address, length, VDPControlPort, VDPDataPort
	...
	move.l	#(VDP_REG.INCR|1)<<16|(VDP_REG.DMASRC_H|$80),VDPControlPort ; VRAM pointer increment: $0001, VRAM fill
	; Forcing the low bit of address to be 1, as described before.
	move.l	#dmaCommand((address)|1,VDP_CMD.VRAM),VDPControlPort
	move.w	#((byte)<<8)|(byte),VDPDataPort
.wait:
	move.w	(a0),d2
	btst	#1,d2
	bne.s	.wait	; busy loop until the VDP is finished filling
	move.w	#(VDP_REG.INCR)|2,(a0) ; VRAM pointer increment back to 2
    endm
